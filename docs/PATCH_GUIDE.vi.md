...
Patch của 1804 nhắm đến ứng dụng bằng bundle identifier ổn định. Gói patch không lưu UUID container vì UUID này thay đổi theo từng máy và mỗi lần cài lại ứng dụng.

## Cấu trúc workspace

Khi tạo patch tên `ABC` cho bundle `com.abc.xyz`, 1804 tạo cây thư mục có thể chỉnh sửa như sau:
...
4. Bấm **Xong**. 1804 sẽ tạo dự án và workspace có thể chỉnh sửa.
5. Mở **Tệp → Workspace 1804 → Patches → tên patch → bundle ID**.
...
3. 1804 tự lấy bundle identifier ổn định và đường dẫn tương đối, sau đó mở bản nháp patch.
4. Lưu bản nháp, mở workspace rồi thay hoặc sắp xếp lại nội dung đã lấy theo nhu cầu.

## Áp dụng và khôi phục

- **Áp dụng** sẽ đồng bộ workspace vào gói `.1804` đã mã hóa, tìm container hiện tại bằng bundle identifier và kiểm tra an toàn từng đường dẫn.
- File đã tồn tại được sao lưu trước khi bị thay. File chưa tồn tại sẽ được thêm mới.
- Toàn bộ lần ghi có nhật ký và kiểm tra lại dữ liệu. Nếu lỗi giữa chừng, 1804 sẽ cố gắng rollback giao dịch.
...
- **Xuất** luôn đồng bộ nội dung mới nhất trong workspace trước khi chia sẻ file `.1804`.
- Có thể nhập từ ứng dụng Tệp bằng cách mở hoặc chia sẻ gói `.1804` sang 1804.
...
- Trên máy hoặc lần cài mới, gói có bảo vệ sẽ hỏi mật khẩu một lần. 1804 lưu content key đã mở khóa trong Keychain; file xuất ra vẫn được mã hóa và luôn gắn với mật khẩu ban đầu.
...
- 1804 từ chối symbolic link, đường dẫn tuyệt đối, thành phần `..`, bundle không hợp lệ và nhiều mục trỏ đến cùng một đích.
